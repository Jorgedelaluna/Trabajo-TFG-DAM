...
public class CoachController {
	...
	public ResponseEntity<List<CoachDTO>> getAllCoaches() {
		List<CoachDTO> lista = coachService.findAll().stream().map(coachMapper::toDTO).toList();
	...
	public ResponseEntity<CoachDTO> getCoachById(@PathVariable Long id) {
		return coachService.findById(id).map(coachMapper::toDTO).map(ResponseEntity::ok)
